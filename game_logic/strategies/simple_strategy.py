...
class SimpleStrategy(Strategy):
    def compute_best_attack_move(self):
        ...
        if self.main_card_sprites_playing_area.cards[0].is_empty():
            card_to_play = min(self.computer_card_sprites_area.cards, key=lambda card: card.value)
        else:
    ...
    def compute_best_defense_move(self):
        ...
        cards_with_same_suit = self.computer_card_sprites_area.get_cards_with_same_suit(bottom_card)
